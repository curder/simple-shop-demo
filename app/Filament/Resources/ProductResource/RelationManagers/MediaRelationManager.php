        ...
        return $table
            ...
            ->columns([
                Tables\Columns\ImageColumn::make('image')
                    ...
                    ->label(__('medias.table.image.label')),
                Tables\Columns\TextColumn::make('file_name')
                    ->limit(30)
                    ->label(__('medias.table.file.label')),
                Tables\Columns\TextColumn::make('custom_properties.name')
                    ->label(__('medias.table.name.label')),
                Tables\Columns\IconColumn::make('custom_properties.primary')
                    ->label(__('medias.table.primary.label'))
                    ->boolean(),
            ])
            ->filters([
                //
            ...
            ->headerActions([
                Tables\Actions\CreateAction::make()
                    ->label(__('medias.actions.create.label'))
                    ->using(function (array $data, string $model): Model {

                        return $this->getOwnerRecord()
                            ->addMediaFromString($data['media']->get())
                            ->usingFileName(
