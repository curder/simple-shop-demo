...
class ProductOptionsWidget extends BaseWidget implements HasActions, HasForms
{
    ...
    protected static string $view = 'filament.product-resource.widgets.product-options';
        ...
        return Action::make('addSharedOption')
            ->form([
                Shout::make('no_shared_components')
                    ->content(
                        __('product_option.widgets.product-options.actions.add-shared-option.form.no_shared_components.label')
                    )
                    ->visible(
                        $options->isEmpty()
                    ),
...
//        };
        ...
        return Action::make(__('products.pages.variants.save.label'))
            ...
            ->action(action: function () {
                ...
                foreach ($this->variants as $variantIndex => $variantData) {
                    ...
                    //                    $basePrice->price = $variantData['price'];
                    //                    $basePrice->save();
    ...
    protected function mapOptionValue(ProductOptionValue $value, bool $enabled = true): array
    {
        return [
            'id' => $value->id,
            'enabled' => $enabled,
