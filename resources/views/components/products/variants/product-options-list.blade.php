...
>
    @foreach($items as $itemIndex => $item)
        <div wire:key="option_{{ $itemIndex }}" x-sortable-option-item="option_{{ $itemIndex }}">
            <div class="grid grid-cols-2 space-x-4">
                <div>
                    <div>
                        <x-filament-forms::field-wrapper.label class="ml-7">
                            {{ __('components.product-options-list.name.label') }}
                        ...
                        <div class="flex w-full space-x-2 mt-1 items-start">
                            ...
                            <div class="grow">
                                ...
                                >
                                    {{ __(
                                        !$item['readonly'] ?
                                            'components.product-options-list.delete-option.label' :
                                            'components.product-options-list.remove-shared-option.label'
                                    ) }}
                                </button>
                            </div>
                        </div>
                ...
                <div class="space-y-1">
                    <x-filament-forms::field-wrapper.label>
                        {{ __('components.product-options-list.values.label') }}
                    </x-filament-forms::field-wrapper.label>
                    <div wire:key="option_values_{{ $itemIndex }}">
                        <x-products.variants.product-option-list-values
    ...
    <x-filament::button color="gray" size="sm" type="button" wire:click.prevent="addRestrictedOption">
        {{ __('components.product-options-list.add-option.label') }}
    </x-filament::button>
</div>